...
// isCJK checks if a rune is a CJK character that can be broken between any characters
func isCJK(c rune) bool {
	return unicode.Is(unicode.Han, c) ||
		unicode.Is(unicode.Hiragana, c) ||
		unicode.Is(unicode.Katakana, c) ||
		unicode.Is(unicode.Hangul, c)
}
...
func (w *WordWrap) Write(b []byte) (int, error) {
	...
	for _, c := range s {
		if c == '\x1B' {
			...
			// any other character
			// Calculate the width of current character using runewidth for CJK support
			charWidth := runewidth.RuneWidth(c)

			// For CJK characters, we can break at any character boundary
			// For non-CJK, we follow the original logic (break only at word boundaries)
			if isCJK(c) {
				// CJK character: check if adding this character would exceed the limit
				if w.lineLen+w.space.Len()+w.word.PrintableRuneWidth()+charWidth > w.Limit {
					// Wrap before adding the character
					if w.word.Len() > 0 {
				...
				// Non-CJK character: add first, then check
				_, _ = w.word.WriteRune(c)

				// Use original logic: add a line break if the current word would exceed
